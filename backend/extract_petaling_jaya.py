"""Petaling Jaya (2016-2024) - 6 districts"""
...
DISTRICTS = [
    ...
    {'name': 'Ara Damansara', 'lat': 3.1167, 'lon': 101.5667},
    {'name': 'Kota Damansara', 'lat': 3.1667, 'lon': 101.5833},
    {'name': 'Taman Tun Dr Ismail', 'lat': 3.1333, 'lon': 101.6333},
    {'name': 'Sea Park', 'lat': 3.1000, 'lon': 101.6333},
]
...
if __name__ == "__main__":
    ...
    for dist in DISTRICTS:
        ...
        for i, dt in enumerate(dates):
            ...
            if r:
                data.append({'city': 'Petaling Jaya', 'district': dist['name'], 'latitude': dist['lat'], 'longitude': dist['lon'], 'date': dt.strftime('%Y-%m-%d'), 'temperature': r['temperature'], 'ndvi': r['ndvi'], 'ndbi': r['ndbi'], 'year': dt.year, 'month': dt.month})
                s += 1
    ...
    if data:
        ...
        pd.DataFrame(data).to_parquet('data/petaling_jaya_2016_2024.parquet', index=False)
