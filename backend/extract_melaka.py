...
DISTRICTS = [
    ...
    {'name': 'Jasin', 'lat': 2.3167, 'lon': 102.4167},
    {'name': 'Alor Gajah', 'lat': 2.3833, 'lon': 102.2167},
    {'name': 'Durian Tunggal', 'lat': 2.2500, 'lon': 102.3000},
    {'name': 'Sungai Udang', 'lat': 2.2833, 'lon': 102.2167},
    {'name': 'Cheng', 'lat': 2.2333, 'lon': 102.2167},
]
...
if __name__ == "__main__":
    ...
    for dist in DISTRICTS:
        ...
        for i, dt in enumerate(dates):
            ...
            if r:
                data.append({'city': 'Melaka', 'district': dist['name'], 'latitude': dist['lat'], 'longitude': dist['lon'], 'date': dt.strftime('%Y-%m-%d'), 'temperature': r['temperature'], 'ndvi': r['ndvi'], 'ndbi': r['ndbi'], 'year': dt.year, 'month': dt.month})
                s += 1
    ...
    if data:
        ...
        pd.DataFrame(data).to_parquet('data/melaka_2016_2024.parquet', index=False)
